...
public class SemanticRoutines{
   ...
   private static HashMap<String,RegisterAllocator> regAlloc = new LinkedHashMap<String,RegisterAllocator>();    
   //YJ-STEP-7: live set updated with each IRnode that is being processed for Tiny code generation
   private static Set<String> liveSet = new HashSet<String>();
   //YJ-STEP7: Use in Allocate 
   private static String lastFreedReg = "";
   private static boolean workaround = false;
   // Stack to hold FOR Loop NODES to handle nested for loops
   private static Stack<ForLoop> forLoops = new Stack<ForLoop>();
   // STACK to keep Track of FOR LOOP LABELS
   private static Stack<String> forLoopLabels = new Stack<String>();
   ...
   // This refers to the Symbol Table of Global Scope in this code. 
   public SymbolMap current_symbol_map = new SymbolMap();
   // To hold labels for Conditional and Unconditional Jumps
  ...
  public static void initialize(){
   ...
   public static void constantToIR(String constant){
        ...
        //Saving Constant to Temporary Mapping
        //temporaryToConstantMap.put(temporary,constant);
   ...
   // PEEK FUNCTIONS ON FOR LOOP LABEL STACK
   public static String peekOnLabelStackFORLOOP(){
      return forLoopLabels.peek();
   
   }
   
   
   public static String peekSecondLabelFORLOOP(){
      String lab1 ="", lab2="";
      if(!forLoopLabels.isEmpty()) {
          lab1 =  ""+forLoopLabels.pop();
      }
      if(!forLoopLabels.isEmpty()) {
          lab2 =  ""+forLoopLabels.pop();
      }
      
      forLoopLabels.push(lab2);
      forLoopLabels.push(lab1);
      return lab2;
   }   
   ...
   public static void makeLabelIRCode(String label, boolean flag){
       currentFunction = FunctionSemanticHandler.currentFuncTable;
         //Save node for CFG mapping .
       IRNode temp = new IRNode("LABEL", "", "",label);
       if(flag) temp.setLoop(true);
       conditionalUnconditionalJumpTarget.put(label,temp);
       currentFunction.functionIRList.add(temp);	
   }
   
   // Handling true false code: As mentioned in manual for Semantics
   
   public static void trueToIR(){
      currentFunction = FunctionSemanticHandler.currentFuncTable;
      String temp1 = getTempMapping("INT");
      currentFunction.functionIRList.add(new IRNode("STOREI", "1","",temp1));
      String temp2 = getTempMapping("INT");
      currentFunction.functionIRList.add(new IRNode("STOREI", "1","",temp2)); 
      currentFunction.functionIRList.add(new IRNode("NE", temp1,temp2,peekOnLabelStack()));     
   
   }
   
   public static void falseToIR(){
      currentFunction = FunctionSemanticHandler.currentFuncTable;  
      String temp1 = getTempMapping("INT");
      currentFunction.functionIRList.add(new IRNode("STOREI", "1", "",temp1));
      //System.out.println("I am here in falseToIR");
      String temp2 = getTempMapping("INT");
      currentFunction.functionIRList.add(new IRNode("STOREI", "0","",temp2)); 
      currentFunction.functionIRList.add(new IRNode("NE", temp1,temp2,peekOnLabelStack()));     
   ...
   public static void initiateFunction(String id){
      ...
      FunctionSymbolTable.setParamCount(0); 
   ...
   public static void setParamCount(){
      ...
      //curr.localParamCount = FunctionSymbolTable.getParamCount();
   }
   public static void enterForLoop(){
      FunctionSemanticHandler.setCurrent(FunctionSemanticHandler.checkLastFunction());
      String startLabel = pushOnLabelStack();
      forLoopLabels.push(startLabel);
      String endLabel = pushOnLabelStack();
      forLoopLabels.push(endLabel);
   }
   ...
   public static void forLoopStart() 
   {
      ...
      forLoops.push(new ForLoop(currentFunction.functionIRList.remove(currentFunction.functionIRList.size()-1),false));
      forLoops.push(new ForLoop(currentFunction.functionIRList.remove(currentFunction.functionIRList.size()-1),false));
      ...
      if(temp.getOperand1().matches("[0-9]*\\.[0-9]+") || temp.getOperand1().matches("[0-9]+")){
         ...
         forLoops.push(new ForLoop(currentFunction.functionIRList.remove(currentFunction.functionIRList.size()-1),true));
   ...
   public static void endForLoop()
   {
      ...
      if(forLoops.peek().isMovable){
       
         //currentFunction.functionIRList.add(for_init);
         currentFunction.functionIRList.add(forLoops.pop().node);
      ...
      makeLabelIRCode(popLabelStack(),false);
   ...
   public static void breakStatement(){
      makeJumpIRCode(peekOnLabelStackFORLOOP());
   }
...
//This function pushes all the dirty registers onto memory and frees them
   public static void pushRegtoMem()
   {
     free("r0");
     free("r1");
     free("r2");
...
//YJ--STEP7: FREE
    ...
    */
      ...
      if ((regStatus) && isLive(regVar))
      {
        ...
        if (regVar.indexOf("$") == -1) //need to check if its a regular variable or not, $ is for stack variables
        {
...
//YJ--STEP7: ALLOCATE
    ...
    */
      ...
      {        
        ...
        else if(lastFreedReg.equals("r3") || lastFreedReg.equals(""))
        {
          free("r0");
			    reg = "r0";
          lastFreedReg = "r0";
      ...
      regAlloc.put(reg, new RegisterAllocator(opr, true));
...
public static String irToTinyOpCoverter2(String operand)
{
...
//YJ-- will convert function local variables to stack offsets (memory locations)
  ...
  public static void tinyInterpreter(ArrayList<IRNode> irList)
  {
    ...
    //NUM->APPROX ISSUE step5 test_for
      ...
      {
        //ensure if operand 1 is in register
        Rx = ensure(irToTinyOpCoverter2(node.getOperand1()));
      ...
      else if (node.getResult().indexOf("$L") != -1)
      {
        //ensure if operand 1 is in register
        Rx = ensure(node.getOperand1()); 
        //System.out.println(";ENSURE returned: "+Rx);
    ...
    else if ( (node.getOpcode().equals("MULTI")) || (node.getOpcode().equals("MULTF")) )
    {
      ...
      Rx = ensure(irToTinyOpCoverter2(node.getOperand1()));
      Ry = ensure(irToTinyOpCoverter2(node.getOperand2()));
    ...
    else if ( (node.getOpcode().equals("ADDI")) || (node.getOpcode().equals("ADDF")) )
    {
      //ensure operands
      //need only one reg, benefit provided by tiny Simulator
      Rx = ensure(irToTinyOpCoverter2(node.getOperand1())); //convert $P, $L, and $R to offsets. Do nothing to $T
      Ry = ensure(irToTinyOpCoverter2(node.getOperand2()));
    ...
    else if ( (node.getOpcode().equals("DIVI")) || (node.getOpcode().equals("DIVF")) )
    {
      ...
      //generate code for operator DIV				
      ...
      regAlloc.get(Rx).setRegStatus(true);
      regAlloc.get(Rx).setRegValue(node.getResult());
      //System.out.println(";DIV--The value of reg: "+Rx +" After this operation is: "+regAlloc.get(Rx).getRegValue());
    ...
    {
      if (node.getResult().indexOf("label") != -1)
      {      
      	//We dont have function label, we have a jump label
        
        //YJ-STEP7: THIS IDENTIFIES THE END OF A BLOCK SO PUSH EVERY DIRTY REG INTO MEMORY
        //AND MAKE EACH DIRTY REG EMPTY
      ...
      else
      {
        ...
        //This means that the label is of a function so initialize funcParamsCounter for that function
      	  ...
      	  currentFuncName = node.getResult();
          TINY_LIST.add(new TinyInstructionNode("label", "", node.getResult()));
    ...
    else if (node.getOpcode().equals("POP"))
    {
      ...
      else if (node.getResult().indexOf("$T") != -1)
      {
        //System.out.println("HELOOOOO");
      	//push temp as register
      	TINY_LIST.add(new TinyInstructionNode("pop", allocate(node.getResult()), ""));
      }
    }
    ...
    else if (node.getOpcode().equals("JSR"))
    {
      ...
      //jsr function name
      TINY_LIST.add(new TinyInstructionNode("jsr", node.getResult(), ""));
