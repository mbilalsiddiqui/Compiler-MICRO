...
program           
    :    'PROGRAM'  id 'BEGIN' pgm_body 'END' { /*FunctionSemanticHandler.printIRCode();*/ControlFlowGraph.dataFlowAnalysis(); ControlFlowGraph.printCFG();ControlFlowGraph.processIRList(); FunctionSemanticHandler.printIRCode();} // YJ addition for step6
    ; 
...
if_stmt           
    :    'IF' {SemanticRoutines.pushOnLabelStack(); SemanticRoutines.pushOnLabelStack(); } '(' cond ')' {SymbolTable.createBlockScope();}  decl stmt_list 
     {SemanticRoutines.makeJumpIRCode(SemanticRoutines.peekSecondLabel());SemanticRoutines.makeLabelIRCode(SemanticRoutines.popLabelStack(),false);} elif_part 'ENDIF' {SemanticRoutines.makeLabelIRCode(SemanticRoutines.popLabelStack(),false);} 
    ;

elif_part         
    :    'ELIF' {SemanticRoutines.pushOnLabelStack();} '(' cond ')' {SymbolTable.createBlockScope();} decl stmt_list  { SemanticRoutines.makeJumpIRCode(SemanticRoutines.peekSecondLabel()); SemanticRoutines.makeLabelIRCode(SemanticRoutines.popLabelStack(),false);} elif_part | else_part
    ;

else_part       
    :    'ELSE' {SemanticRoutines.pushOnLabelStack();} {SymbolTable.createBlockScope();} decl stmt_list {SemanticRoutines.makeJumpIRCode(SemanticRoutines.peekSecondLabel()); SemanticRoutines.makeLabelIRCode(SemanticRoutines.popLabelStack(),false);} | 
    ;
...
basic_cond        
    :    expr compop expr {SemanticRoutines.addConditionalStatementIRCode($compop.text,map);} | 'TRUE'{SemanticRoutines.trueToIR();}  | 'FALSE' {SemanticRoutines.falseToIR();} 
    ;
...
for_stmt       
    :    'FOR' {SemanticRoutines.enterForLoop();}'(' init_stmt ';' {SemanticRoutines.makeLabelIRCode(SemanticRoutines.peekSecondLabel(),true);} cond ';' incr_stmt ')' {SymbolTable.createBlockScope();SemanticRoutines.forLoopStart();}  decl aug_stmt_list  'ENDFOR'{SemanticRoutines.endForLoop();}
    ;
...
aug_stmt         
    :    base_stmt | aug_if_stmt | for_stmt | 'CONTINUE'';' | 'BREAK'{SemanticRoutines.breakStatement();}';'
    ;

aug_if_stmt       
    :    'IF'  {SemanticRoutines.pushOnLabelStack(); SemanticRoutines.pushOnLabelStack(); }'(' cond ')' {SymbolTable.createBlockScope();} decl aug_stmt_list  {SemanticRoutines.makeJumpIRCode(SemanticRoutines.peekSecondLabel());SemanticRoutines.makeLabelIRCode(SemanticRoutines.popLabelStack(),false);} aug_elif_part 'ENDIF' {SemanticRoutines.makeLabelIRCode(SemanticRoutines.popLabelStack(),false);} 
    ;

aug_elif_part     
    :    'ELIF'  {SemanticRoutines.pushOnLabelStack();} '(' cond ')' {SymbolTable.createBlockScope();} decl aug_stmt_list {SemanticRoutines.makeJumpIRCode(SemanticRoutines.peekSecondLabel());SemanticRoutines.makeLabelIRCode(SemanticRoutines.popLabelStack(),false);} aug_elif_part | aug_else_part
    ;

aug_else_part     
    :    'ELSE'  {SemanticRoutines.pushOnLabelStack();}{SymbolTable.createBlockScope();} decl aug_stmt_list  {SemanticRoutines.makeJumpIRCode(SemanticRoutines.peekSecondLabel());SemanticRoutines.makeLabelIRCode(SemanticRoutines.popLabelStack(),false);}| 
    ;
